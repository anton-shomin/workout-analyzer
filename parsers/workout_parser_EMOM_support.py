...
def _parse_scheme(content, workout_type='Unknown'):
    ...
    # NEW: Detect EMOM
    scheme_type = scheme.get('type', workout_type).upper()
    if 'EMOM' in scheme_type:
        scheme['is_emom'] = True
        print(f"   ℹ️  Detected EMOM workout")
...
def _parse_exercises(content, scheme):
    ...
    is_emom = scheme.get('is_emom', False)
    ...
    for line in lines:
        ...
        # Pattern 3: Just number at START (for EMOM: "4 тяги")
        ...
        if sets_reps_match:
            ...
            if is_emom:
                # EMOM: multiply by rounds
                reps = reps_per_round * rounds
                print(f"   ℹ️  EMOM: {clean_line} = {reps_per_round} × {rounds} раундов = {reps}")
            else:
                reps = reps_per_round
